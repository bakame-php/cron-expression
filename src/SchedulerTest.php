...
final class SchedulerTest extends TestCase
{
    ...
    {
        // Test next run date
        $cron = Expression::fromString($expression);
    ...
    {
        $cron = Expression::fromString('0 15 * * 3'); //Wednesday at 15:00

        $cronUTC = new Scheduler($cron, 'UTC', InitialDatePresence::EXCLUDED); //Wednesday at 15:00
        $cronAms = new Scheduler($cron, 'Europe/Amsterdam', InitialDatePresence::EXCLUDED); //Wednesday at 15:00
        $cronTok = new Scheduler($cron, new DateTimeZone('Asia/Tokyo'), InitialDatePresence::EXCLUDED); //Wednesday at 15:00
    ...
    {
        $cron = Expression::fromString('0 15 * * 3');
        $cronUTC = new Scheduler($cron, 'UTC', InitialDatePresence::EXCLUDED); //Wednesday at 15:00
        $cronAms = new Scheduler($cron, new DateTimeZone('Europe/Amsterdam'), InitialDatePresence::EXCLUDED); //Wednesday at 15:00
        $cronTok = new Scheduler($cron, new DateTimeZone('Asia/Tokyo'), InitialDatePresence::EXCLUDED); //Wednesday at 15:00
        $date = '2014-01-01 15:00'; //Wednesday
        $utc = new DateTimeZone('UTC');
        $amsterdam = new DateTimeZone('Europe/Amsterdam');
    ...
    public function testItFailsWithAWrongTimezone(): void
    {
        ...
        new Scheduler('* * * * *', 'Foo/Bar', InitialDatePresence::EXCLUDED);
    }
    ...
    public function testCanGetPreviousRunDates(): void
    {
        ...
        $cron = Scheduler::fromSystemTimezone(Expression::fromString('* */2 * * *'));
        $next = $cron->run('now');
        $two = $cron->run('now', 1);
    ...
    public function testYieldRunsForwardWillFailWithNegativeRecurrences(): void
    {
        ...
        iterator_to_array(
            Scheduler::fromSystemTimezone(Expression::fromString('*/2 * * * *'))
    ...
    {
        $cron = Scheduler::fromSystemTimezone(Expression::fromString('* * * * *'));
    ...
    {
        $cron = Scheduler::fromSystemTimezone(Expression::fromString('0 0 27 JAN *'));
    ...
    public function testIssue20(): void
    {
        ...
        $scheduler = Scheduler::fromSystemTimezone(Expression::fromString('* * * * SAT#2'));
        ...
        $scheduler = Scheduler::fromSystemTimezone(Expression::fromString('* * * * SUN#3'));
    ...
    {
        $scheduler = Scheduler::fromSystemTimezone(Expression::fromString('00 * * * *'));
        self::assertTrue($scheduler->isDue(new DateTime('2014-04-07 00:00:00')));

        $scheduler = Scheduler::fromSystemTimezone(Expression::fromString('01 * * * *'));
    ...
    {
        $scheduler = Scheduler::fromSystemTimezone(Expression::fromString('* * * */123 *'));
        self::assertTrue($scheduler->isDue(new DateTime('2014-04-07 00:00:00')));
    ...
    public function testRecognisesTimezonesAsPartOfDateTime(): void
    {
        ...
        $scheduler = new Scheduler(expression: '0 7 * * *', timezone: $tzCron, initialDatePresence: InitialDatePresence::EXCLUDED);
    ...
    {
        $initial = new Scheduler('0 7 * * *', 'Africa/Nairobi', InitialDatePresence::EXCLUDED);
    ...
    {
        $scheduler = new Scheduler('0 7 * * *', 'Africa/Nairobi', InitialDatePresence::EXCLUDED);
    ...
    public function testRunReturnsADateTimeImmutableObject(): void
    {
        ...
        $scheduler = new Scheduler('0 15 2 * 3', 'Africa/Kampala', InitialDatePresence::EXCLUDED);
    ...
    public function testDSTFixForDateIntervalUpdates(): void
    {
        ...
        $scheduler = new Scheduler('0 1 * * 0', $tz, InitialDatePresence::INCLUDED);
